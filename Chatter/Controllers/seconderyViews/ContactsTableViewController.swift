...
class ContactsTableViewController: UITableViewController, UISearchResultsUpdating, UserTableViewCellDelegate {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !isGroup {
            ...
            if !checkBlockedStatus(withUser: userToChat) {
                ...
                chatVC.membarIds = [FUser.currentId(), userToChat.objectId]
                chatVC.membersToPush = [FUser.currentId(), userToChat.objectId]
                chatVC.chatRoonmId = startPrivateChat(user1: FUser.currentUser()!, user2: userToChat)
    ...
    func setupButtons() {
        ...
        if isGroup {
            ...
            let inviteButton = UIBarButtonItem(image: UIImage(named: "nearMe"), style: .plain, target: self, action: #selector(self.inviteUsers))
            
            
            let nextButton = UIBarButtonItem(title: "Next", style: .plain, target: self, action: #selector(self.nextButtonPressed))

           
            self.navigationItem.rightBarButtonItems = [ nextButton,inviteButton]
